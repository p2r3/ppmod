...
**Note: this is an experimental feature and may not function as expected in many cases.**
```squirrel
  ppmod.fwrite(path, string)
```
The first argument is a path to the file to write to, relative to the `portal2` directory. The second argument is the string to write. Note that this doesn't overwrite the file, but instead appends the given string to the end. There is currently no good way of clearing a file entirely within unmodded VScript, and even this method employs several hacks which may not work on all systems.

This works by using the `con_logfile` console command in combination with a simple VScript `print`. In order to get rid of the restriction of needing the logfile to end with `.log`, the given path is bloated just enough to overflow the buffer by 4 characters. This is known to work consistently on Linux, but has not been thoroughly tested on Windows.
